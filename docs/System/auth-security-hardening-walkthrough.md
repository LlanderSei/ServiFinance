...
- Lockout email alerts are sent when SMTP is configured and the recipient has a linked Google account. Account lockouts notify the locked account; network lockouts notify linked root SuperAdmins for root-origin events or linked tenant Owner/Administrator users for tenant/customer-origin events.
