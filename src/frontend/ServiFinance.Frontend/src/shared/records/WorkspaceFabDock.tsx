...
export function WorkspaceFabDock({ actions }: WorkspaceFabDockProps) {
  ...
  return (
    <div
      className={`authed-workspace__fab pointer-events-none fixed right-3 bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-[70] inline-flex items-center justify-end gap-[0.55rem] lg:absolute lg:right-4 lg:bottom-4 lg:z-[20] lg:justify-start ${isExpanded ? "" : "is-collapsed"}`}
    >
      <button
        type="button"
      ...
      <div
        className={`pointer-events-auto inline-flex max-w-[calc(100vw-5rem)] items-center gap-[0.55rem] overflow-x-auto rounded-full border border-base-300/70 bg-base-100 p-2 shadow-xl transition-[width,opacity,padding] duration-200 lg:max-w-none lg:overflow-visible ${isExpanded ? "" : "w-0 overflow-hidden border-transparent p-0 opacity-0 shadow-none pointer-events-none"}`}
